 ...
 */
app.controller('AppCtrl', ['$rootScope', '$scope', '$state', '$translate', '$localStorage', '$window', '$document', '$timeout', 'cfpLoadingBar', 'principal', 'UserServices' ,
function($rootScope, $scope, $state, $translate, $localStorage, $window, $document, $timeout, cfpLoadingBar, principal, UserServices) {
	...
	var $win = $($window);
	   ...
	   $rootScope.filled=function(){

        
   		$state.go('app.auth.user.modify', {id:$rootScope.userLogged.id});
	...
	$rootScope.$on('$stateChangeSuccess', function(event, toState, toParams, fromState, fromParams) {
		...
		// keep userLogged full when reload
	...
	$rootScope.isAdmin = function(value){
    ...
    //go back come il tasto indietro del browser
    $rootScope.doTheBack = function() {
	  window.history.back();
	};

	$rootScope.isListMe = function(){
		if($state.includes("**.listme.**")) return true;
		return false
	}
